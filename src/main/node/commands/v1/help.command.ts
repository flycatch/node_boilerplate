...
import * as commands from '../index.commands';
...
export class HelpCommand implements Command {
    ...
    cliName: string = config.app.name;
    cliDesc: string = config.app.desc;
    ...
    public generateCommandHelp(name: string, command: Command): string {
        let help = '\n  ';
        ...
        if (command.flags && Object.keys(command.flags).length != 0) {
            ...
            for (let flagName in command.flags) {
